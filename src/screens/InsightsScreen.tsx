...
export default function InsightsScreen() {
  ...
  function getEventStyle(type: string, message: string) {
    switch (type) {
      case 'cry': return { gradient: gradients.coral, icon: 'volume-high' as const };
      case 'sleep': return { gradient: gradients.lavender, icon: (message.includes('asleep') ? 'moon' : 'sunny') as const };
      default: return { gradient: gradients.primary, icon: 'walk' as const };
    }
  }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      {events.length === 0 ? (
        <View style={styles.emptyState}><Text style={[styles.emptyText, { color: colors.textMuted }]}>{t('noEvents')}</Text></View>
      ) : events.map((event, index) => {
        const style = getEventStyle(event.type, event.message);
        return (
          <View key={index} style={[styles.eventCard, { backgroundColor: colors.card }]}>
            <LinearGradient colors={style.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.iconCircle}>
...
const styles = StyleSheet.create({
  ...
  eventCard: { flexDirection: 'row', borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm, alignItems: 'center', ...shadow.card },
  iconCircle: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', marginRight: spacing.md },
  eventContent: { flex: 1 },
  eventMessage: { ...typography.body, fontWeight: '500' },
